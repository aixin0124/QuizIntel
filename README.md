...
当前产品入口已经按“官网 + 个人控制台 + API 服务 + 平台超级管理员后台”拆分：

- `/`：产品官网首页，介绍问卷平台、个人空间、可选协作、API 服务和平台治理。
- `/login`、`/register`：普通账号登录与注册。注册会创建个人问卷空间和初始 Token 额度。
- `/console`：个人工作区，原有问卷导入、AI 包装、发布、答卷、分析、导出等能力都在这里。
- `/api-service`：独立的 API 服务页，展示个人账号剩余 Token、已用 Token、调用记录和额度申请；毕设阶段不接入真实支付。
- `/platform/login`：平台超级管理员独立登录入口。
- `/platform`：平台超级管理员后台，负责全部账号查看、账号冻结/恢复、个人资源用量、平台配置、Token 审批和内容审核。
- `/share/{token}`：问卷公开填写页，仍然只通过分享令牌访问。

账号与协作规则：

- `platform` / `10124`：平台超级管理员演示账号，只能从 `/platform/login` 登录。
- `aixin` / `123456`：普通个人账号演示账号，可以从 `/login` 登录并管理自己的问卷。
- 每个注册用户默认拥有自己账号下全部问卷的最大权限，不需要先创建其他资源容器。
- 协作是可选能力：用户可以邀请已经注册的成员，并选择“仅查看”或“可编辑”。
- 被邀请用户会在“协作成员”中收到消息，接受后才能访问；问卷所有者还可以继续按单份问卷逐项授权。

Token 采用个人工作区额度模型：问卷解析、互动包装和最终数据分析会估算并扣减 Token；调用失败会返还已扣额度。用户通过 `/api-service` 提交额度申请，由平台超级管理员审核后发放。生成记录会保存估算 Token、模型、耗时和状态，便于展示每份问卷的调用成本。
...
1. 启动项目并打开管理后台。
2. 打开官网首页，点击“登录”进入个人控制台；演示账号为 `aixin`，密码为 `123456`。平台后台从“平台管理”或 `/platform/login` 进入，演示账号为 `platform` / `10124`。
