...
import { BarChart3, CheckCircle2, Database, FileDown, FileSpreadsheet, FileText, KeyRound, LayoutDashboard, Link2, LoaderCircle, LockKeyhole, LogOut, Play, Plus, RotateCcw, ShieldCheck, Sparkles, Trash2, UploadCloud, Users, X } from "lucide-react";
import "./styles.css";
...
function CreateWorkspace({ token, onCreated }: { token: string; onCreated: (id: number, survey: WrappedSurvey) => void }) {
  const [sourceText, setSourceText] = useState(demoQuestionnaire); const [wjxUrl, setWjxUrl] = useState(""); const [questions, setQuestions] = useState<SurveyQuestion[]>([]); const [brandGoal, setBrandGoal] = useState("洞察用户选购或组装台式主机时的预算分层、使用场景、性能取舍与决策方式，为产品定位和配置推荐提供依据"); const [themeHint, setThemeHint] = useState("你的隐藏行动风格"); const [message, setMessage] = useState(""); const [loading, setLoading] = useState(false); const [generating, setGenerating] = useState(false);
  async function parseSurvey() { setLoading(true); setMessage(""); try { const data = await adminPostJson<{ questions: SurveyQuestion[] }>("/api/surveys/parse", { content: sourceText, file_name: sourceText.trim().startsWith("[") ? "survey.json" : "survey.csv" }, token); setQuestions(data.questions); setBrandGoal(buildDefaultBrandGoal()); setMessage(`已识别 ${data.questions.length} 道题，并已将问卷标题写入调研目标。`); } catch (error) { setMessage(getErrorMessage(error)); } finally { setLoading(false); } }
  async function parseWjx() { setLoading(true); setMessage(""); try { const data = await adminPostJson<{ title: string; questions: SurveyQuestion[] }>("/api/surveys/parse-wjx", { url: wjxUrl }, token); setQuestions(data.questions); setBrandGoal(buildDefaultBrandGoal(data.title)); setMessage(`已从问卷星导入“${data.title}”，识别 ${data.questions.length} 道题，并已将问卷标题写入调研目标。`); } catch (error) { setMessage(getErrorMessage(error)); } finally { setLoading(false); } }
  async function uploadFile(file: File) { setLoading(true); setMessage(""); try { const formData = new FormData(); formData.append("file", file); const response = await fetch("/api/surveys/upload", { method: "POST", headers: { "X-Admin-Token": token }, body: formData }); if (!response.ok) throw new Error(await readError(response)); const data = (await response.json()) as { questions: SurveyQuestion[] }; setQuestions(data.questions); setBrandGoal(buildDefaultBrandGoal()); setMessage(`已从文件识别 ${data.questions.length} 道题，并已将问卷标题写入调研目标。`); } catch (error) { setMessage(getErrorMessage(error)); } finally { setLoading(false); } }
  ...
  return <section><GenerationOverlay visible={generating} /><div className="page-title"><div><p className="eyebrow">CREATE A STUDY</p><h2>创建互动包装</h2><p>导入原始问卷作为研究参考，让 AI 围绕主题重新设计互动题和分析维度。</p></div><Sparkles size={26} /></div>{message && <div className="notice">{message}</div>}<div className="workspace two"><div className="panel import-panel"><div className="section-heading"><div><h2><Link2 size={19} /> 导入问卷</h2><p>优先粘贴问卷星公开转发链接，解析后可继续编辑。</p></div><span className="step-number">01</span></div><label htmlFor="wjx-url">问卷星转发链接</label><div className="url-input-row"><input id="wjx-url" type="url" value={wjxUrl} onChange={(event) => setWjxUrl(event.target.value)} placeholder="https://v.wjx.cn/vm/xxxxx.aspx" /><button className="primary import-action" onClick={() => void parseWjx()} disabled={loading || !wjxUrl.trim()}><Link2 size={16} /> 解析链接</button></div><p className="field-hint">支持 v.wjx.cn / www.wjx.cn 的公开问卷链接；解析后可修改题目、选项和题型。</p><div className="import-divider"><span>或使用文件导入</span></div><label className="fileBox"><input type="file" accept=".csv,.json" onChange={(event) => event.target.files?.[0] && void uploadFile(event.target.files[0])} />选择 CSV / JSON 文件</label><label htmlFor="survey-source">JSON / CSV 内容</label><textarea id="survey-source" value={sourceText} onChange={(event) => setSourceText(event.target.value)} /><button onClick={() => void parseSurvey()} disabled={loading}>解析文本</button></div><div className="panel"><div className="section-heading"><h2><Sparkles size={19} /> AI 包装设定</h2><span className="step-number">02</span></div><label>调研目标</label><textarea className="short" value={brandGoal} onChange={(event) => setBrandGoal(event.target.value)} /><p className="field-hint">导入问卷后会自动填入问卷标题，也可以继续补充调研目标。</p><label>测评主题</label><input value={themeHint} onChange={(event) => setThemeHint(event.target.value)} placeholder="例如：年轻人的口红消费风格、周末旅行决策风格" /><button className="primary" onClick={() => void wrapSurvey()} disabled={!questions.length || !brandGoal.trim() || !themeHint.trim() || loading}>生成互动包装</button></div></div><QuestionEditor questions={questions} onChange={setQuestions} /></section>;
}
