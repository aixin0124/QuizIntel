...
import { BarChart3, BookOpen, CheckCircle2, Database, ExternalLink, FileDown, FileSpreadsheet, FileText, KeyRound, LayoutDashboard, Link2, LoaderCircle, LockKeyhole, LogOut, Play, Plus, RotateCcw, ShieldCheck, Sparkles, Trash2, Users, X } from "lucide-react";
import "./styles.css";
...
function CreateWorkspace({ token, onCreated }: { token: string; onCreated: (id: number, survey: WrappedSurvey) => void }) {
  ...
  async function importTemplate(template: SurveyTemplate) { setLoading(true); setMessage(""); try { const data = await adminPostJson<{ title: string; source_url: string; questions: SurveyQuestion[] }>("/api/surveys/import-template", { url: template.sourceUrl }, token); setQuestions(data.questions); setBrandGoal(`${template.goal}。来源问卷：${data.title}`); setThemeHint(template.themeHint); setWjxUrl(data.source_url); setTemplateDialogOpen(false); setMessage(`已从推荐模板“${template.title}”抓取真实问卷“${data.title}”，识别 ${data.questions.length} 道题。`); } catch (error) { setMessage(getErrorMessage(error)); } finally { setLoading(false); } }
